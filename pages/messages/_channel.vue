				...
				<div class="row flex-">
					...
					<div class="col-sm col-md-12">
						...
						<ul class="list-unstyled">
							<li v-for="user in subscribers" :key="user.id" class="mb-2">
...
export default {
  ...
  head() {
    // TODO: think way to find opponent user
    // [th]
    // const title = !this.channel.owner
    //   ? 'undefined user'
    //   : this.user.id === this.channel.owner.id
    const title = `Room ${this.channel.id}`
    return {
      // title: this.user.id === this.channel.owner.id
      // ?
      title
    }
  },
  computed: {
    ...
    subscribers() {
      const {
        owner: { content, id, name, username }
      } = this.channel
      return [
        {
          avatar_image: content.avatar_image.link,
          id,
          name,
          username
        },
        ...this.channel.acl.write.user_ids
      ]
    }
  },
  components: {
